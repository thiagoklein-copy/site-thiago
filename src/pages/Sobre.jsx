...
const FORMACAO = [
  { name: 'Copywriter Certificado AAA — Empiricus', detail: 'Copy Camp' },
  { name: 'Mentoria Além da Copy', detail: '2024' },
  { name: 'Marketing — PUCRS', detail: 'em andamento', badge: 'Em curso' },
]
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(
      (entries) => {
        entries.forEach((e, i) => {
          if (e.isIntersecting) {
            timelineEl.classList.add('sobre-timeline-active')
            setTimeout(() => e.target.classList.add('sobre-tl-visible'), i * 200)
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(
      (entries) => {
        entries.forEach((e, i) => {
          if (e.isIntersecting) {
            setTimeout(() => e.target.classList.add('sobre-cert-visible'), i * 140)
            obs.unobserve(e.target)
          }
        })
      },
          ...
          <div ref={timelineRef} className="sobre-timeline relative">
            {EXPERIENCE.map((item, i) => (
              ...
              >
                ...
                <div className="sobre-tl-right relative pl-0 sm:pl-10">
